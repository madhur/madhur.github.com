...
description: >-
  ...
  not, when to require a passkey, how long a session should live, and where
  secrets actually belong.
tags:
  - Homelab
  - Docker
  - Authelia
...
A couple of months ago I [consolidated my homelab auth stack on Authelia](/blog/2027/07/01/homelab-auth-authelia-authentik-pocketid.html). That migration answered the "which tool" question. It didn't answer the harder questions that come after: which apps should speak OIDC, which should stay on forward-auth, what happens when the IdP is down, and how aggressive 2FA and session expiration should be when the only user is me.
