---
layout: blog-post
title: "Turning My Homelab PC Into the House Router for a Day, Just to Find One Bandwidth Hog"
excerpt: "My ISP router has no per-device stats, and I don't want to root or replace it. So for one day I made my Arch homelab box the LAN's DHCP server and NAT gateway, hairpinning every device's traffic back out through the same router — no rewiring, fully scripted, fully reversible. Then I kept a piece of it: a journald-backed flow logger, a hairpin-aware dedupe, and a push-alerting big-flows report that outlived the audit."
