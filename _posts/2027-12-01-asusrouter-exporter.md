---
layout: blog-post
title: "asusrouter-exporter: Prometheus Metrics for an Asuswrt-Merlin Router, No Cloud API Required"
excerpt: "There's no official Prometheus exporter for Asuswrt-Merlin, so I SSH in and read /proc myself. TDD against real router fixtures, counter-reset handling for reboots, and — since the connected-client data is genuinely mine — a git-filter-repo pass to scrub household devices out of the history before open-sourcing it."
